...
Backend keys: `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` (LLM), `DATABASE_URL`, `REDIS_URL`, `APIFY_API_TOKEN`, `FRONTEND_ORIGIN` (CORS **and** the base for emailed `?verify=`/`?reset=` links), `SENTRY_DSN`, `MCP_API_KEY` (must match `VITE_MCP_API_KEY`), `FEATURE_CHAT_SESSIONS`, `FREE_CHAT_MESSAGE_LIMIT` (free Career-Mentor messages/user, default 5; free users also get exactly one saved chat thread), `PAYMENT_PROVIDER` (`mock`|`razorpay`|`stripe`), Stripe (`STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_*`, optional `STRIPE_SUCCESS_URL`/`STRIPE_CANCEL_URL`).

Email + auth: signup enqueues an email-verification message and login offers password reset, both via the Postgres email queue + dispatcher (`EMAIL_DISPATCHER_ENABLED`). Delivery uses `EMAIL_PROVIDER` (`none`|`postmark`|`resend`) + `EMAIL_FROM` and, for Resend, `RESEND_API_KEY`. **While `EMAIL_PROVIDER=none` no mail is sent** — the dispatcher logs each `?verify=`/`?reset=` URL at INFO instead (testable via `docker compose logs app`). Set `EMAIL_PROVIDER=resend` + `RESEND_API_KEY` to send real mail. Verification is a soft nag (app stays usable; a dismissible banner offers resend); reset tokens are single-use, expire (24h verify / 1h reset), and rotate the access token to invalidate old sessions. Social sign-in (Google/LinkedIn OAuth) has been removed — email/password is the only path.
