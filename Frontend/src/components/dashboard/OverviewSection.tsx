  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-stretch gap-8">
        <div className="flex-1 bg-dark-card p-8 rounded-xl border border-dark-border border-l-4 border-l-dark-accent">
          <div className="flex items-center gap-2 mb-4">
            <h2 className="text-xl font-bold text-dark-textPri">
              Executive Brief
            </h2>
            <SourceBadge source="ai_inferred" size="sm" />
          </div>
          <p className="text-dark-textSec text-lg leading-relaxed">
            {executiveBrief}
          </p>
        </div>
        <div className="flex-shrink-0 w-full md:w-auto">
          <Card className="h-full p-6 flex flex-col items-center justify-center min-w-[280px]">
            <h3 className="text-sm font-bold text-dark-textMuted uppercase tracking-wider mb-4">
            ...
            {/* Score Narrative (F18) */}
            {scoreNarrative && (
              <p className="text-xs text-dark-textMuted text-center mt-3 leading-relaxed max-w-[250px]">
                {scoreNarrative}
              </p>
            )}
          </Card>
      ...
      {companyAnalysis && (
        <Card className="p-8 bg-dark-elevated border-dark-border">
          ...
          <p className="text-dark-textSec leading-relaxed">{companyAnalysis}</p>
        </Card>
      )}
            ...
            {competitorIntel.map((comp, i) => (
              <Card key={i} className="p-5 border-l-4 border-l-amber-400">
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-bold text-dark-textPri">{comp.name}</h4>
                  <Badge variant="warning">Threat</Badge>
                </div>
                <p className="text-sm font-medium text-dark-textPri mb-1">
                  {comp.aiInitiative}
                </p>
                <p className="text-xs text-dark-textMuted">Impact: {comp.impact}</p>
              </Card>
            ))}
          </div>
        )}
                  ...
                  return (
                    <div key={i}>
                      <div className="flex justify-between text-sm mb-2">
                        <span className="font-medium text-dark-textSec">{bench.metric}</span>
                      </div>
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-dark-textMuted w-20 flex-shrink-0">
                            {companyName}
                          </span>
                          ...
                          <span
                            className={`font-bold text-sm w-12 text-right ${isAhead ? 'text-dark-green' : 'text-dark-red'}`}
                          >
                            {bench.userValue}
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-dark-textMuted text-sm w-12 text-right">
                            {bench.industryAvg}
                          </span>
                        </div>
                      </div>
                      <p className="text-xs text-dark-textMuted mt-1.5 italic">{bench.insight}</p>
                    </div>
                  );
                })}
              </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {scoreFactors.map((factor, index) => (
            ...
            >
              <Card className="p-6 h-full flex flex-col transition-shadow">
                <div className="flex justify-between items-start mb-3">
                  <span className="font-bold text-dark-textPri text-lg">{factor.name}</span>
                      ...
                      <span
                        className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${
                          factor.confidence === 'high'
                            ...
                            : 'bg-dark-amber/10 text-dark-amber'
                        }`}
                      >
                        {factor.confidence}
                      </span>
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-semibold text-dark-textSec w-10 text-right">
                      {factor.value}
                    </span>
                  </div>
                </div>
                <div className="border-t border-dark-border pt-3 mt-auto">
                  <p className="text-dark-textSec text-sm leading-relaxed">
                    <span className="font-semibold text-dark-textPri block mb-1">
                      Analysis:
                    </span>
